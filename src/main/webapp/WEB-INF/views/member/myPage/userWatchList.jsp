...
<html>
    ...
    <body>
        ...
        <main class="main_fri">
            ...
            <section class="section_content">
                ...
                <!-- 찜 목록 생성 공간 -->
                <div class="section_content_div1_watchList"></div>
                <!-- page -->
    ...
    <script>
        ...
        // profile user 이름 변동
        UserName = () => {
            const lastnameElement = document.querySelector('.usrLastname');
            const firstnameElement = document.querySelector('.usrFirstname');

            if (lastnameElement && firstnameElement) {
                lastnameElement.textContent = userInfo.usrLastname;
                firstnameElement.textContent = userInfo.usrFirstname;
            }
        };
        UserName();
        console.log(userInfo);
        ...
        let userWatchListCount;
        if ('${userWatchListCount}' !== '') {
            userWatchListCount = Number('${userWatchListCount}');
        }

        const pageCount = Math.ceil(userWatchListCount / itemsPerPage);
        ...
        if (currentPage > 1)
            pagination.innerHTML +=
                `
        <a class="first pagination page_button" href="/member/myPage/userWatchList?currentPage=1&itemsPerPage=9">&lt;&lt;</a>
        <a class="prev pagination page_button" href="/member/myPage/userWatchList?currentPage=` +
                (currentPage - 1) +
                `1&itemsPerPage=9">&lt;</a>
        `;
        ...
        for (let i = 0; i < pageCount; i++) {
            ...
            <a class="pagination page_button` +
                ...
                `" href="/member/myPage/userWatchList?&currentPage=` +
                (i + 1) +
                `&itemsPerPage=9">` +
                (i + 1) +
                `</a>
        ...
        if (currentPage < pageCount)
            pagination.innerHTML +=
                `
        <a class="next pagination page_button" href="/member/myPage/userWatchList?currentPage=` +
                (currentPage + 1) +
                `&itemsPerPage=9">&gt;</a> 
        <a class="last pagination page_button" href="/member/myPage/userWatchList?currentPage=` +
                pageCount +
                `&itemsPerPage=9">&gt;&gt;</a>
        `;

        // wishList
        const section_content_div1_watchList = document.querySelector('.section_content_div1_watchList');

        let userWatchListByPage;

        // 유무 확인 if 문으로
        if ('${userWatchListByPage}' !== '') {
            userWatchListByPage = JSON.parse('${userWatchListByPage}');

            let html = ''; // String형 변수로 선언
            for (let i = 0; i < userWatchListByPage.length; i++) {
                html +=
                    `
                <div class="card_product1">
                        <a href="/product/` +
                    userWatchListByPage[i].vhsProusrselid +
                    `/ ` +
                    userWatchListByPage[i].vhsProcode +
                    `/">
                            <span class="wrap_thumb">
                                <img cuimg="" uselazyloading="" class="img_g" src="` +
                    (userWatchListByPage[i].productImage.length === 0 ? '/res/img/whitebox_logo_p.png' : userWatchListByPage[i].productImage[0].pigPath) +
                    `" />
                            </span>
                            <span class="wrap_info"> <strong> </strong></span></a
                        ><strong><a class="txt_brand">` +
                    userWatchListByPage[i].proName +
                    `</a> </strong>
                        <div>
                            <a class="txt_product">` +
                    userWatchListByPage[i].proInfo +
                    `</a>
                        </div>
                        <div class="txt_price">
                            <strong> ¥<span class="txt_yen">` +
                    userWatchListByPage[i].proPrice +
                    `</span> </strong>
            ...
            section_content_div1_watchList.innerHTML = html; // 반복문으로 상품 불러온 것을 저장
        }

        console.log(userWatchListByPage);
        console.log(userWatchListCount);
    </script>
</html>
